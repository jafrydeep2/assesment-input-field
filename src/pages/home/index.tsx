...
function Home() {
  ...
  const [tblData, setTblData] = useState(dummyList);
  const [isEditing, setIsEditing] = useState(false);
  const [selectedEditIndex, setSelectedEditIndex] = useState<number|any>('');
  ...
  const handleSubmit = () => {
    ...
    if (data?.name && data?.sector && data?.agree) {
      ...
      setTblData([...tblData, ...tbNewData]);
      // console.log(tblData);
      localStorage?.setItem(
        'tblData',
        JSON.stringify([...tblData, ...tbNewData]),
      );
      handleClearSession();
    }
  };
  ...
  useEffect(() => {
    if (typeof window !== undefined) {
      ...
      setTblData(JSON.parse(localStorage?.getItem('tblData') as any) || dummyList);
      console.log(data);
    }
  }, []);

  // handle edit
  const handleEdit = (item: any, mainIndex: number) => {
    setData(item);
    setIsEditing(true);
    setSelectedEditIndex(mainIndex);
    ...
    //     }) as any
    //   );
    //   console.log(tblData?.map((e, index) => {
    //     if (mainIndex === index) {
    //       return {
    //         ...e,
    //         name: data?.name,
    ...
    //   }) as any);
    //   // console.log(tblData);
    //   localStorage?.setItem('tblData', JSON.stringify([...tblData,...tbNewData]));
    // }
  };
  const handleEditSubmit = () => {
    ...
    if (data?.name && data?.sector && data?.agree) {
      ...
      setTblData(tblData?.map((e, index) => {
      ...
      localStorage?.setItem(
        'tblData',
        JSON.stringify(tblData?.map((e, index) => {
          if(index === selectedEditIndex){
            return{
              ...e,
              name:data?.name,
  ...
  // handle remove
  const handleRemove = (item:any, mainIndex: number) => {
    setTblData(tblData?.filter((e,index) => index !== mainIndex));
    localStorage?.setItem(
      'tblData',
      JSON.stringify(tblData?.filter((e,index) => index !== mainIndex)),
    );

  };

  return (
    <div style={{ marginTop: 100 }} className="block lg:flex">
      {/* if adding data */}
      {!isEditing && (
      ...
      {/* Table / List */}
      <div className="max-w-xl m-auto content-center bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 flex-1 flex-col my-2">
        <div className="-mx-3 md:flex mb-6">
          <div className="md:w-full px-3">
            <h3 className="text-md mb-2">Stored Data</h3>
            {tblData?.map((e, index) => (
              <div
                key={index}
                className="todo-container text-gray-700 text-center mt-2"
              >
                <div className="flex justify-between bg-blue-100 shadow mb-2">
                  <span className="w-full text-left leading-loose p-2 pl-10 cursor-pointer">
                    {e?.name}
                  </span>
                  {e?.hasAccess && <button
                    className="close-btn p-2 pr-8 text-md cursor-pointer text-blue-700"
                    onClick={() => handleEdit(e, index)}
                  >
                    Edit
                  </button>}
                  {e?.hasAccess && <button
                    className="close-btn p-2 pr-8 text-md cursor-pointer text-red-600"
                    onClick={() => handleRemove(e, index)}
                  >
                    Delete
                  </button>}
                </div>
